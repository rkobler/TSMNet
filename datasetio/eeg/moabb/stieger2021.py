...
class Stieger2021(PreprocessedDataset):

    BASE_URL = "https://ndownloader.figshare.com/files/"

    def __init__(self, interval = [0, 3], channels = None, srate = None, sessions = None):
        super().__init__(
        ...
        for f in fsn.keys():
            ...
            if sbj == subject and ses in self.sessions:
                ...
                if not os.path.exists(fpath):
                    pooch.retrieve(Stieger2021.BASE_URL + fsn[f], reg[fsn[f]], f, path, downloader=pooch.HTTPDownloader(progressbar=True))
                spath.append(fpath)
        return spath
    ...
    def _get_single_subject_data(self, subject):
        ...
        for fn in fname:
            ...
            eeg_ch_names = [ch.replace('Z', 'z').replace('FP','Fp') for ch in eeg_ch_names]
            # extract all standard EEG channels
            montage = mne.channels.make_standard_montage("standard_1005")
            channel_mask = np.isin(eeg_ch_names, montage.ch_names)
            ch_names = [ch for ch, found in zip(eeg_ch_names, channel_mask) if found]  + ["Stim"]
            ...
            for i in range(container.data.shape[0]):
                ...
                if container.TrialData[i].artifact == 0 and (container.TrialData[i].triallength + 2) > self.interval[1]:
            ...
            for idx in badchanidxs:
                # badchan = [eeg_ch_names[idx-1] for idx in badchanidxs]
            ...
            if len(raw.info['bads']) > 0:
                log.info(f'Record {subject}/{session} (subject/session): bad channels that will be interpolated: {raw.info["bads"]}')
